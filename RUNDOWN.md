...
Seeing as the Epson Moverio BT-300 was capable of very little in terms of AR and was unable to render our projects requirements to satisfactory levels, we ported the project over to the Microsoft Hololens as with our research into technologies capable of rendering our project, it seemed to be the only headset with the capacity for of spatial mapping and tringulation. Two features necessary to remove the head bobbing issue. Much of the development done previously was repurposed but in terms of user settings we had to start from scratch and use the Headsets User Interface (UI) to create a new settings screen for the user.
